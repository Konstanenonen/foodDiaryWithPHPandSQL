  ...
  if (!isset($_SESSION['userid'])) 
  {
    if (isset($_POST['loging-in'])) {
      ...
      if (!empty($username) && !empty($password)) {
        // Fetch the userid, username and password from the database
        ...
        if (mysqli_num_rows($data) == 1) {
          ...
          $error_msg = '<p style="text-align: center;"><strong>Sorry, the username/password you entered is not correct. Try again!</strong></p>';
        }
      }
      else {
        // The username/password weren't entered so set an error message
        $error_msg = '<p style="text-align: center;"><strong>Sorry, you must enter your username and password to log in. Try again!</strong></p>';
      }
    }
  }
  // If the session data is empty, show any error message and the log-in form; otherwise confirm the log-in
  if (empty($_SESSION['userid'])) {
    echo '<p class="error style="text-align: center;"><trong>' . $error_msg . '</strong></p>';
  }
?>
